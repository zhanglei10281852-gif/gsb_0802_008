...
export function readLineageRequest (value) {
  ...
  if (!Number.isInteger(value.expectedRevision) || value.expectedRevision < 0) {
    throw new ApiError(400, 'invalid_revision', 'expectedRevision must be a non-negative integer')
  }
  if (!Array.isArray(value.relations) || value.relations.length === 0 || value.relations.length > 500) {
    throw new ApiError(400, 'invalid_relations', 'relations must contain between 1 and 500 items')
  }
  const relations = value.relations.map((relation) => {
    if (!relation || typeof relation !== 'object' || Array.isArray(relation)) {
      throw new ApiError(400, 'invalid_relation', 'every relation must be an object')
    }
    if (typeof relation.version !== 'string' || relation.version.trim().length === 0) {
      throw new ApiError(400, 'invalid_relation', 'relation.version is required')
    }
    if (typeof relation.parent !== 'string' || relation.parent.trim().length === 0) {
      throw new ApiError(400, 'invalid_relation', 'relation.parent is required')
    }
    let application = scope.application
    if ('application' in relation) {
      if (typeof relation.application !== 'string' || relation.application.trim().length === 0) {
        throw new ApiError(400, 'invalid_relation', 'relation.application must be a non-empty string')
      }
      application = relation.application.trim()
    }
    let platform = scope.platform
    if ('platform' in relation) {
      if (typeof relation.platform !== 'string' || relation.platform.trim().length === 0) {
        throw new ApiError(400, 'invalid_relation', 'relation.platform must be a non-empty string')
      }
      platform = relation.platform.trim()
    }
    return { version: relation.version.trim(), parent: relation.parent.trim(), application, platform }
  })
  return { application: scope.application, platform: scope.platform, expectedRevision: value.expectedRevision, relations }
}
...
export function readBatchRequest (value) {
  ...
  if (!Array.isArray(value.items) || value.items.length === 0 || value.items.length > 100) {
    throw new ApiError(400, 'invalid_batch', 'items must contain between 1 and 100 requests')
  }
  return value.items
}

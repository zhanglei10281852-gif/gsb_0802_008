...
function sameBoundary(childIdentity, parentIdentity) {
  return (
    childIdentity.application === parentIdentity.application &&
...
export class BundleRegistry {
  ...
  put(body) {
    ...
    if (previous.bundles.has(key)) {
      throw new ApiError(
        ...
        "A bundle for this release already exists; re-uploading cannot replace lineage",
      );
    }
      ...
      if (!previous.bundles.has(parentKey)) {
        if (this.#existsAcrossBoundary(previous, identity, parentVersion)) {
          throw new ApiError(
            ...
            "parent release exists in a different application or platform",
          );
        }
        throw new ApiError(
          400,
          "unknown_version",
          "parent release is not registered",
        );
      }
    }
    ...
    this.#recordHistory({
      ...
      kind: "publish",
      changes: parentKey
        ? [{ ...identity, fromVersion: null, toVersion: parentVersion }]
        : [],
      parents,
      digests: this.#liveDigests(bundles),
    });
  ...
  adjustLineage(body) {
    ...
    if (previous.revision !== expectedRevision) {
      throw new ApiError(
        412,
        "revision_mismatch",
        `expected revision ${expectedRevision} but registry is at ${previous.revision}`,
      );
    }
    ...
    this.#recordHistory({
      ...
      changes: plan.changes,
      parents: plan.parents,
      digests: this.#liveDigests(previous.bundles),
    });
    return {
      revision: nextRevision,
  ...
  rollbackLineage(body) {
    ...
    if (targetRevision > previous.revision) {
      throw new ApiError(
        ...
        "target revision does not exist yet",
      );
    }

    const targetParents = this.#parentsAtRevision(targetRevision);
    const relationships = this.#diffParentsForRollback(
      previous,
      targetParents,
      targetRevision,
    );
  ...
  previewGarbageCollection(body) {
    const expectedRevision = this.#readGcExpectedRevision(body);
    const state = this.#state;
    const collectible = this.#computeCollectible(state);
    return {
      ok: true,
      baseRevision: state.revision,
      nextRevision: state.revision + 1,
      ...
      collectible: collectible.map(({ digest, mappingCount }) => ({
        digest,
        mappingCount,
      })),
    };
  ...
  collectGarbage(body) {
    ...
    const collectible = this.#computeCollectible(previous);
    if (collectible.length === 0) {
      return { revision: previous.revision, collected: [] };
    }

    const artifacts = new Map(previous.artifacts);
    for (const { digest } of collectible) artifacts.delete(digest);
    ...
    return {
      revision: nextRevision,
      collected: collectible.map(({ digest, mappingCount }) => ({
        digest,
        mappingCount,
      })),
    };
  ...
  #computeCollectible(state) {
    const protectedDigests = new Set();

    for (const descriptor of state.bundles.values()) {
      protectedDigests.add(descriptor.digest);
    }

    for (const entry of this.#history) {
      for (const digest of entry.digests) protectedDigests.add(digest);
    }

    for (const [, leaseRevision] of this.#leases) {
      const digests = this.#digestsAtRevision(state, leaseRevision);
      for (const digest of digests) protectedDigests.add(digest);
    }

    const collectible = [];
    for (const [digest, artifact] of state.artifacts) {
      if (!protectedDigests.has(digest)) {
        collectible.push({ digest, mappingCount: artifact.mappings.length });
      }
    }
    return collectible;
  }

  #digestsAtRevision(state, revision) {
    if (revision >= state.revision) return this.#liveDigests(state.bundles);
    for (let i = this.#history.length - 1; i >= 0; i--) {
      if (this.#history[i].revision <= revision) {
        return this.#history[i].digests;
      }
    }
    return new Set();
  }
  ...
  #liveDigests(bundles) {
    ...
    for (const descriptor of bundles.values()) digests.add(descriptor.digest);
    return digests;
  }
  ...
  #planLineageChange(state, relationships) {
    ...
    for (const { identity, parentVersion } of relationships) {
      ...
      if (!state.bundles.has(childKey)) {
        if (this.#existsAcrossBoundary(state, identity, identity.version)) {
          throw new ApiError(
            400,
            "cross_boundary_reference",
            "child release belongs to a different application or platform",
          );
        }
        throw new ApiError(
          400,
          "unknown_version",
          `child release ${identity.version} is not registered`,
        );
      }

      const fromVersion = parents.has(childKey)
        ? parseBundleKey(parents.get(childKey)).version
        : null;

      if (parentVersion === null) {
        if (fromVersion !== null)
          changes.push({ ...identity, fromVersion, toVersion: null });
      ...
      if (childKey === parentKey) {
        throw new ApiError(
          ...
          "a release cannot be its own parent",
        );
      }
      if (!state.bundles.has(parentKey)) {
        ...
        throw new ApiError(
          400,
          "unknown_version",
          `parent release ${parentVersion} is not registered`,
        );
      }
    ...
    const affectedReleases = this.#computeAffectedReleases(
      state,
      state.parents,
      parents,
      changedKeys,
  ...
  #computeAffectedReleases(state, oldParents, newParents, changedKeys) {
    ...
    for (const key of candidates) {
      ...
      const oldPositions = this.#collectChainPositions(
        state,
        identity,
        oldParents,
      );
      const newPositions = this.#collectChainPositions(
        state,
        identity,
        newParents,
      );
      ...
      for (const posKey of allKeys) {
        ...
        const before = this.#resolveAgainstParents(
          state,
          identity,
          generated,
          oldParents,
        );
        const after = this.#resolveAgainstParents(
          state,
          identity,
          generated,
          newParents,
        ...
        if (
          before.resolvedFrom !== after.resolvedFrom ||
  ...
  #diffParentsForRollback(state, targetParents, targetRevision) {
    ...
    for (const [childKey, descriptor] of state.bundles) {
      ...
      const targetParentVersion = targetParentKey
        ? parseBundleKey(targetParentKey).version
        : null;
      const currentParentKey = state.parents.get(childKey);
      const currentParentVersion = currentParentKey
        ? parseBundleKey(currentParentKey).version
        : null;
  ...
  #existsAcrossBoundary(state, identity, version) {
    for (const existingKey of state.bundles.keys()) {
      ...
      if (existing.version === version && !sameBoundary(identity, existing))
        return true;
    }
    return false;
  }
    ...
    while (current) {
      if (current === startKey || visited.has(current)) {
        throw new ApiError(
          400,
          "lineage_cycle",
          "lineage change would introduce a cycle",
        );
      }
      visited.add(current);
